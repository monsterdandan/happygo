	...
	let val=getCookieValue("userName");
	if(val){
		$(".loginBox").html("<a href='../index.html'>"+val+"中午好！</a><a href='' class='exit'>退出</a>");
...
//轮播图
	...
	//限时抢购
		var xsqgArr=$(".headline dl");
		var xsqgimgArr=$(".headline dt img");
		for(let i=0;i<xsqgArr.length;i++){
			xsqgArr[i].onmouseover=function(){
				xsqgArr[i].style.border="2px solid #c41f3a";
				xsqgimgArr[i].style.transform="scale(1.1)";
				xsqgimgArr[i].style.transition="all 0.6s"
			}
			xsqgArr[i].onmouseout=function(){
				xsqgArr[i].style.borderColor="#fff";
				xsqgimgArr[i].style.transform=""
			}
		}

		let dlNine=$(".goodsNine dl");
		let dlFive=$(".goodsFive dl");
		for(let i=0;i<dlNine.length;i++){
			dlNine[i].style.left=203*i+"px";
		};
		$(".xsqg_next").click(function(){
			
			for(let i=0;i<dlNine.length;i++){
				dlNine[i].style.left=(-189*(5-i))+"px";
				dlNine[i].style.transition="left 1s";
			}
		})
		$(".xsqg_prev").click(function(){
			for(let i=0;i<dlNine.length;i++){
				dlNine[i].style.left=189*i+"px";
				dlNine[i].style.transition="left 1s";
			}
		})
		/*let yesterdayArr=$(".live_hidden").children();
		...
		$(".nineTime").mouseenter(function(){
			$(".goodsNine")[0].style.display="block";
			$(".goodsFive")[0].style.display="none";
			$(".goodsEight")[0].style.display="none";
			$(".nine").addClass("borderLi");
			$(".five").removeClass("borderLi")
			$(".eight").removeClass("borderLi")
			for(let i=0;i<dlNine.length;i++){
				dlNine[i].style.left=203*i+"px";
				dlNine[i].style.transition="left 1s";
			}
		})
		$(".fiveTime").mouseenter(function(){
			$(".goodsFive")[0].style.display="block";
			$(".goodsNine")[0].style.display="none";
			$(".goodsEight")[0].style.display="none";
			$(".five").addClass("borderLi");
			$(".nine").removeClass("borderLi")
			$(".eight").removeClass("borderLi")
		...
		$(".eightTime").mouseenter(function(){
			$(".goodsEight")[0].style.display="block";
			$(".goodsNine")[0].style.display="none";
			$(".goodsFive")[0].style.display="none";
			$(".eight").addClass("borderLi");
			$(".five").removeClass("borderLi")
			$(".nine").removeClass("borderLi")
		...
		/*$(".today").click(function(){
			...
			$(".goodsFive")[0].style.display="block";
			for(let i=0;i<todayArr.length;i++){
				dlFive[i].style.left=189*i+"px";
				dlFive[i].style.transition="left 1s";
			}
		})*/
